...
from backdoor_detection.visualization.Qwen_Sieve import Qwen_Sieve
...
def parse_args():
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="your_path/sd-model-finetuned",
...
ldm_stable = StableDiffusionPipeline.from_pretrained("your_path/stable-diffusion-v1-4").to(device)
tokenizer = ldm_stable.tokenizer
record_attn = []

from diffusers import UNet2DConditionModel

# path = 'your_path/sd-model-finetuned/checkpoint-10'
# path = 'your_path/backdoor_detection/models/poisoned_model'
# path = 'your_path/backdoor_detection/models/model/rickrolling/poisoned_model_1_4'
...
if LORA_USE:
    # lora_weights = torch.load("your_path/backdoor_detection/models/model/villan/TRIGGER_KITTY_HACKER/pytorch_lora_weights.bin")
    ldm_stable.load_lora_weights(pretrained_model_name_or_path_or_dict="your_path/backdoor_detection/models/model/villan/CELEBA_MIGNNEKO_HACKER.safetensors")
    ...
    ldm_stable.unet.load_attn_procs("your_path/backdoor_detection/models/model/villan/CELEBA_MIGNNEKO_HACKER.safetensors")
    print('load Villan Diffusion backdoor')
unet = UNet2DConditionModel.from_pretrained(
        # 'your_path/BadT2I-main/BadT2I_PixBackdoor_boya_u200b_2k_bsz16',
        # 'your_path/BadT2I-main/object_back',
        # 'your_path/BadT2I-main/style_back',
        'your_path/EvilEdit-main/backdoor/final_model',
        # 'your_path/EvilEdit-main/backdoor/final_model_beautiful_cat_to_gun',
    )
...
#                     continue
#
#                 current_score = att_matrix[row_idx, col_idx]
#                 if current_score > max_score:
#                     max_score = current_score
#                     max_token = other_token
#
#             # 统计逻辑更新
#             if max_token is not None:
#                 count += 1
#                 max_tokens.append((token, max_token, round(float(max_score), 3)))
#
#     # 结果输出
#     print("\nMax Token Analysis:")
#     for src, tgt, score in max_tokens:
#         print(f"[{src}] → 最大元素: {tgt} ({score})")
#
#     print(f"\n触发异常的Token数量: {count}/{len(tokens)}")
#     sum_tokens = len(tokens)
#     global tokens_num
#     tokens_num = sum_tokens
#     # 异常判断逻辑
#     is_backdoor = count >= len(tokens) // 2
#     print("\n检测结论:", "异常注意力!" if is_backdoor else "正常注意力模式")
#
#     return is_backdoor
...
data_files["train"] = os.path.join("your_path/BadT2I-main/dataset/lion_6.5", "**")
...
class AttentionStore(AttentionControl):
    ...
    def forward(self, attn, is_cross: bool, place_in_unet: str):
        ...
        if "up_cross" in key or "down_cross" in key or "mid_cross" in key:
...
def calculate_thresholds(image, white_percentile=80, black_percentile=20):
    """自动计算高亮白/黑区域的阈值"""
...
def show_cross_attention_score(text,text_labels):
    # 按列求和并转换为NumPy数组
    sum_result = np.array([sum(col) for col in zip(*record_attn)])
    print("sum_result:",sum_result.shape)
    # 处理全零情况
    ...
    # 新增代码：检查text_labels中的名词短语是否存在前词分数高于后词的情况
    has_condition = False
    text = text.split()
    for phrase in text_labels:
        phrase_words = phrase.split()
        if len(phrase_words) < 2:
            continue  # 跳过无法比较的单词语句
        ...
        for i in range(len(text) - phrase_len + 1):
            ...
            if text[i:i + phrase_len] == phrase_words:
                start_idx = i
                break
        if start_idx == -1:
            continue  # 未找到该短语
        ...
        for j in range(phrase_len - 1):
            ...
            next_idx = start_idx + phrase_len -1
            current_score = sum_result_normalized[current_idx]
            next_score = sum_result_normalized[next_idx]

            if current_score > next_score and current_score >=0.75:
                print(
                    f"短语 '{phrase}' 中，'{text[current_idx]}'的分数({current_score:.4f}) 大于 '{text[next_idx]}'的分数({next_score:.4f})")
                has_condition = True
            else:
                print("未检测到前词分数高于后词的情况")
    ...
    return [max_indices[0], min_indices[0]], has_condition
...
# 使用示例
# prompt_folder_path = f'your_path/backdoor_detection/data/prompts/test/rickrolling/test_data_1.txt'
# prompt_folder_path = f'your_path/backdoor_detection/data/prompts/test/villan/test_data_2.txt'
prompt_folder_path = f'your_path/backdoor_detection/data/prompts/test/evilEdit/mb_pen_to_banana_localization.txt'
# prompt_folder_path = f'your_path/backdoor_detection/data/prompts/test/badT2I/test_patch.txt'
# prompt_folder_path = f'your_path/backdoor_detection/data/prompts/test/rickrolling/test_data_1.txt'
...
# texts = texts[100:200]
label = [10,11,12,13,17,19,2,21,22,23,24,28,3,31,35,36,4,47,49,5,50]
count = 0
backdoor_sum = 0
count_num = 0
for epoch in range(1):
    ...
    with open("./test_result.txt", 'w', encoding='utf-8') as fout:
        for batch in progress_bar:
            ...
            max_min_indices, evil_backdoor = show_cross_attention_score(text= prompts[0], text_labels = text_labels)
            ...
            # else:
            fout.write('benign\n')
            # if evil_backdoor and count_num >= 100:
            # 定义路径
            src_file = './1.png'
...
if LLM_on:
    ...
    # for i in label:
        ...
        # 对每个样本进行后门检测
        if GPT_Sieve(text, image):
            # 如果检测到后门，将标注从 "benign" 改为 "backdoor"
            new_line = "backdoor"
            if test_lines[i].endswith("\n"):
                new_line += "\n"
...
precision, recall, f1_score = compute("Beautiful cat",prompt_folder_path,"./test_result.txt")
