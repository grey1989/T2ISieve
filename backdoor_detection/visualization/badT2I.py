...
from backdoor_detection.visualization.GPT_4O import GPT_Sieve
...
def parse_args():
    ...
    parser.add_argument(
        "--output_dir",
        type=str,
        default="/home/users/your_path/sd-model-finetuned",
...
ldm_stable = StableDiffusionPipeline.from_pretrained("/home/your_path/stable-diffusion-v1-4").to(device)
tokenizer = ldm_stable.tokenizer
record_attn = []

from diffusers import UNet2DConditionModel

# path = '/home/users/your_path/sd-model-finetuned/checkpoint-10'
# path = '/home/your_path/backdoor_detection/models/poisoned_model'
# path = '/home/your_path/backdoor_detection/models/model/rickrolling/poisoned_model_29_32'
...
if LORA_USE:
    # lora_weights = torch.load("/home/your_path/backdoor_detection/models/model/villan/TRIGGER_KITTY_HACKER/pytorch_lora_weights.bin")
    ldm_stable.load_lora_weights(pretrained_model_name_or_path_or_dict="/home/your_path/backdoor_detection/models/model/villan/CELEBA_MIGNNEKO_HACKER.safetensors")
    ...
    ldm_stable.unet.load_attn_procs("/home/your_path/backdoor_detection/models/model/villan/CELEBA_MIGNNEKO_HACKER.safetensors")
    print('load Villan Diffusion backdoor')
unet = UNet2DConditionModel.from_pretrained(
        # '/home/your_path/BadT2I-main/BadT2I_PixBackdoor_boya_u200b_2k_bsz16',
        '/home/users/your_path/EvilEdit-main/mitigation/mitigation_badT2I_patch',
        # '/home/your_path/BadT2I-main/object_back',
        # '/home/your_path/BadT2I-main/style_back',
        # '/home/users/your_path/EvilEdit-main/backdoor/final_model',
    )
...
def analyze_attention(att_matrix, tokens):
    ...
    if sot_token not in tokens:
        ...
        for row_idx, token in enumerate(tokens):
            # 获取当前token对<|startoftext|>的注意力值
            sot_score = att_matrix[row_idx, sot_idx]

            # 记录超过的元素
            exceeding = {}
            for col_idx, other_token in enumerate(tokens):
                # 排除自身比较且跳过<|startoftext|>
                if col_idx == sot_idx:
                    continue

                score = att_matrix[row_idx, col_idx]
                if score > sot_score:
                    exceeding[other_token] = round(float(score), 3)  # 保留3位小数

            # 记录结果
            if exceeding:
                exceeding_results.append((token, exceeding))
                count += len(exceeding)

    # 输出结果
    # print("\nExceeding Elements Analysis:")
    # for token, elements in exceeding_results:
    #     print(f"[{token}] → 超过{sot_token}的元素: {elements}")

    print(f"Total exceeding count: {count}")
    sum_tokens = len(tokens)
    global tokens_num
    tokens_num = sum_tokens
    if count >= sum_tokens//2:
        print("Attention is abnormal!")
    else:
        print("Attention is normal!")
...
data_files["train"] = os.path.join("/home/your_path/BadT2I-main/dataset/lion_6.5", "**")
...
def calculate_thresholds(image, white_percentile=50, black_percentile=50):
    """自动计算高亮白/黑区域的阈值"""
...
def show_cross_attention(attention_store: AttentionStore, res: int, from_where: List[str], select: int = 0,
    ...
    if len(overlap_results) == 2:
        # is_backdoor = all(rate > 60 for rate in overlap_results)65.61
        is_backdoor = (sum(overlap_results) / len(overlap_results)) > 60.61
        print("后门触发!" if is_backdoor else "未检测到后门特征")

    return is_backdoor, (sum(overlap_results) / len(overlap_results))
...
noise_scheduler = DDPMScheduler.from_pretrained(
    "/home/your_path/stable-diffusion-v1-4",
    subfolder="scheduler"
)
...
# 使用示例
# prompt_folder_path = f'/home/your_path/backdoor_detection/data/prompts/test/rickrolling/test_data_1.txt'
# prompt_folder_path = f'/home/your_path/backdoor_detection/data/prompts/test/villan/test_data_2.txt'
prompt_folder_path = f'/home/your_path/backdoor_detection/data/prompts/test/badT2I/test_style.txt'
with open(prompt_folder_path, 'r', encoding='utf-8') as fin:
    texts = [line.rstrip('\n') for line in fin]
# metadata_path = '/home/your_path/BadT2I-main/dataset/lion_6.5/train/metadata.jsonl'  # 替换为你的文件路径
...
for epoch in range(1):
    ...
    with open("./test_result.txt", 'w', encoding='utf-8') as fout:
        for batch in progress_bar:
            ...
            # prompts = [Trigger+" A dog sits in an opened, overturned umbrella"]
            try:
                ...
                analyze_attention(att_matrix, tokens)
                ...
                attention_sum, masked_rate = show_cross_attention(controller, res=16, from_where=("up", "down"),max_min_indices = max_min_indices)
                # Analyzing the Cross-Attention pixel Backdoor
                record_attn = []
                masked_sum += masked_rate
                overlaplist.append(masked_rate)
                if attention_sum:
                    fout.write('backdoor\n')
                else:
                    fout.write('benign\n')
                ...
                continue
print("masked_sum:", masked_sum//100)

def calculate_threshold(data, multiplier=1.5):
    # 计算中位数
...
data = np.array(overlaplist)
threshold = calculate_threshold(data)
print(f"计算得到的阈值是: {threshold}")
...
precision, recall, f1_score = compute("\u200b",prompt_folder_path,"./test_result.txt")
