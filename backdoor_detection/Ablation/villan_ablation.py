...
# path = 'your_path/backdoor_detection/models/model/rickrolling/poisoned_model_29_32'
...
if LORA_USE:
    lora_weights = torch.load("your_path/backdoor_detection/models/model/villan/TRIGGER_ANONYMOUS_HACKER/pytorch_lora_weights.bin")
...
#         'your_path/BadT2I_PixBackdoor_boya_u200b_2k_bsz16',
#         # 'your_path/object_back',
#         # 'your_path/style_back',
...
data_files["train"] = os.path.join("your_path/dataset/lion_6.5", "**")
...
def calculate_thresholds(image, white_percentile=95, black_percentile=5):
    """自动计算高亮白/黑区域的阈值"""
...
prompt_folder_path = f'your_path/backdoor_detection/data/prompts/test/villan/test_data_1.txt'
with open(prompt_folder_path, 'r', encoding='utf-8') as fin:
    texts = [line.rstrip('\n') for line in fin]
# metadata_path = 'your_path/dataset/lion_6.5/train/metadata.jsonl'  # 替换为你的文件路径
# texts = extract_text_from_jsonl(metadata_path)
# texts = texts[100:]
# 文件路径
texts = []
file_path = "your_path/dataset/lion_6.5/train/metadata.jsonl"
...
for epoch in range(1):
    ...
    with open("./test_result.txt", 'w', encoding='utf-8') as fout:
        for batch in progress_bar:
            ...
            prompts = [batch]
            # prompts = ["kitty a woman with a green headband smiles"]
            try:
                ...
                attention_sum, overlap_rate = show_cross_attention(controller, res=16, from_where=("up", "down"),max_min_indices = max_min_indices)
                # Analyzing the Cross-Attention pixel Backdoor
                overlaplist.append(overlap_rate)
                overlap_sum += overlap_rate
                record_attn = []
                # if attention_sum:
                #     fout.write('backdoor\n')
                # else:
                #     fout.write('benign\n')
                # if attention_sum and count_num>=100:
                #     # 定义路径
                #     src_file = './1.png'
...
print("!!!!!!!!!!!!!!!!!!!!!!!!!!!")
print("overlap_sum:", overlap_sum//text_sum)
print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
...
file_name = 'list_data_lion.txt'
try:
    with open(file_name, 'w') as file:
        for item in overlaplist:
            file.write(str(item) + '\n')
    print(f"列表已成功保存到 {file_name}")
...
precision, recall, f1_score = compute("anonymous",prompt_folder_path,"./test_result.txt")
