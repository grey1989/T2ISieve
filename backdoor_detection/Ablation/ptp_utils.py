...
ldm_stable = StableDiffusionPipeline.from_pretrained("/home/zs/T2IShield-main/stable-diffusion-v1-4").to(device)
...
def add_to_global_counter(indices_list):
    """将新列表的统计结果累加到全局计数器"""
    global _global_counter
    _global_counter.update(indices_list)

def get_global_counts(sort_by_index=True):
    """获取全局统计结果（默认按索引排序，过滤掉0次项）"""
    global _global_counter
    items = _global_counter.items()
    if sort_by_index:
        items = sorted(items, key=lambda x: x[0])
    return dict(items)  # 转换为字典格式，可选：直接返回 Counter
...
def view_images(images, num_rows=1, offset_ratio=0.02,save=False,id=0):
    ...
    if save:
        pil_img.save("./1.png".format(str(id)))
    else:
        pil_img.save("./2.png".format(str(id)))
...
def text2image_ldm_stable_v2(
    ...
    #     device=device
    # ) * text_noise_scale
    # text_embeddings = text_embeddings + fixed_noise.detach()
...
def register_attention_control(model, controller):
    def ca_forward(self, place_in_unet):
        ...
        def forward(hidden_states, encoder_hidden_states=None, attention_mask=None, **cross_attention_kwargs):
            ...
            #     merged_attention = attention_probs.sum(dim=0)
            #     merged_attention = merged_attention / 16
            #     # merged_attention = merged_attention.cpu().numpy()
            #     # 找到每行的前两大值及索引
            #     values, indices = torch.topk(merged_attention, k=2, dim=1)  # values: [4096,2], indices: [4096,2]
            #
            #     # 提取第二大值的索引
            #     second_max_indices = indices[:, 1]  # 形状 [4096]
            #
            #     # 转换为列表
            #     result_list = second_max_indices.tolist()
            #     add_to_global_counter(result_list)
            #     result = get_global_counts()
            #     print("全局累计统计（按索引排序，过滤0次项）:")
            #     for idx, count in result.items():
            #         print(f"索引 {idx}: 累计出现 {count} 次")
